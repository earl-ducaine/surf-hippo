...
#+cmu
(ext:file-comment
  "$Header: /home/CVS-cmucl/src/clx/buffer.lisp,v 1.8 1999/03/16 23:37:36 pw Exp $")
  ...
  `(let ((%reply-buffer ,reply-buffer)
     ...
     (let* ((buffer-boffset (the array-index ,(or index 0)))
       ...
       (declare (type array-index buffer-boffset)
		(type buffer-bytes buffer-bbuf)
		(ignorable buffer-boffset buffer-bbuf))
       ,@body)))
...
(declaim (notinline buffer-new-request-number))
(proclaim '(notinline buffer-new-request-number))
