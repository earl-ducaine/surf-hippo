...
(defun update-virtual-polyline (gob update-info bbox-1 bbox-2  &optional (total-p NIL))
	...
	(dotimes (n (the fixnum (gv gob :next-available-rank)))
	  ...
	  (when (and (opal::bbox-valid-p item-bbox)
	    ...
	    (let* ((item-values (the simple-array (aref (the simple-array item-array) n)))
		   (lstyle (virtual-polyline-line-style item-values))
		   (fstyle (virtual-polyline-filling-style item-values))
		   (point-list (virtual-polyline-point-list item-values)))
	      (gem::x-draw-lines-fast a-window point-list function lstyle fstyle))))))
