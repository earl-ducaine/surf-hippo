...
(defstruct (node-double-floats (:type (vector double-float)))
  ...
  (const-current 0.0d0	:type double-float) ; The time-invariant component (Seg G-AXIAL * v-leak)
  (dvdt-n 0.0d0 :type double-float)
  (dvdt-n-1 0.0d0 :type double-float))
...
(defstruct (membrane-element-iv-values (:type (vector double-float)))
  (conductance 0.0d0 :type double-float)	; microsiemans or units of cm3/s
  (gbar 0.0d0 :type double-float)		; microsiemans or units of cm3/s
  ...
  (gbar/perm-reference-value 0.0d0 :type double-float)		; microsiemans or units of cm3/s
  )
