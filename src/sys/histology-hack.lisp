...
;;; This code supplanted by cell-graphics-hack lisp source files.
		  ...
		  `((:histology-scale-bar-background ,opal:rectangle
		    ...
		    (:histology-scale-bar-label ,opal:text
						...
						(:string ,(o-formula (if (gvl :parent :window :include-histology-scale-bar-label)
								       ...
								       "")))
						(:left ,(o-formula 
							 (round (+ (gvl :parent :x-left) (/ (- (gvl :parent :x-right) (gvl :parent :x-left) (gvl :width)) 2)))))
...
(defun create-histology-window (&key width height type prototype background-color)
  (let* ((win (create-instance nil histology-window
    ...
    (when prototype
      ...
      (set-histology-window-angle-scale-parameters win (g-value prototype :theta) (g-value prototype :phi) (g-value prototype :scale)))
...
(defun get-histology-child-window (parent &optional parented &key exclude-auxiliary-type)
  (let* ((parent-child-number (g-value parent :child-number))
	 (child (GET-HISTOLOGY-WINDOW (g-value parent :type)
				      (if parented (format nil "~a-~d" (g-value parent :title) (1+ parent-child-number)) (find-histology-window-title))
...
(defun view-angle-comment-p (win) (and (g-value win :view-angle-comment-p) (not (and (= 0 (g-value win :theta)) (= 0 (g-value win :phi))))))

(defun add-view-angle-comment (win)
  (add-comment
   win (when (view-angle-comment-p win)
	 (format nil "Viewing theta/phi: ~a/~a" (round (rad-to-deg (g-value win :theta))) (round (rad-to-deg (g-value win :phi)))))))
...
(defun reset-histology-xfrm (win center-x center-y width height scale)
  ...
  (setf (aref (g-value win :current-xfrm) 2 0) (s-flt (/ (- center-x) scale))) ; x-shift in pixels
  (setf (aref (g-value win :current-xfrm) 2 1) (s-flt (/ (- center-y) scale))) ; y-shift in pixels
  (TRANSFER-CURRENT-XFRM-TO-XFRM-SLOTS win)
  (when width
...
(defun add-line (x1 y1 x2 y2 agg &key dimensions-in-pixels style stipple-percent color (thickness 0) (where :front))
  ...
  (let* ((win (g-value agg :parent :window)) 
	 (line-style (get-add-line-line-style color stipple-percent
					      (round (/ thickness (g-value agg :parent :window :scale)))
...
(defun add-arrow (x y length agg &key (thickness 4) ; (head-length 1.0)
  ...
  (multiple-value-bind (pix-x pix-y)
    ...
    (let* ((arrow (create-instance
		   ...
		   (:x1 x) (:x2 x) (:y2 y) (:y1 (round (+ y (/ length (g-value agg :parent :window :scale))))))))
      (opal:add-component agg  arrow))))

(defun graphics-window-cells (win) (no-nils (loop for cell in (g-value win :cells) collect (element cell 'cell))))
   ...
   (progn
     ...
     (let* ((x-y (x-y-histology-win-inv (nth 2 points) (nth 3 points) window))
	    ...
	    (distance (when (g-value window :last-pointer-xy)
			(cartesian-distance-float (car x-y) (cadr x-y) (nth 0 (g-value window :last-pointer-xy) (nth 1 (g-value window :last-pointer-xy))))))) 
       	 (update-running-comment window
...
(defun zoom (interactor point-list)
  (histo-interactor-wrapper
   (when (graphics-window-cells (g-value interactor :window))
...
(defun unzoom (interactor final-obj-over)
  (declare (ignore final-obj-over))
  (histo-interactor-wrapper
   (when (and (graphics-window-cells window) (g-value window :last-xfrms))
