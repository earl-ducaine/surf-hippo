...
(defun show-image (image &key (win nil window-supplied) title left top height width (update t)
		   crop image-left image-top (HORIZONTAL-SIDE-BORDER 0) (VERTICAL-SIDE-BORDER 0))
  "Returns a window, default WIN, that shows the bitmap IMAGE image. IMAGE can be either a Garnet bitmap schema or a filename. If
IMAGE is nil then browse. Image size is adjusted by HORIZONTAL-SIDE-BORDER and VERTICAL-SIDE-BORDER. Window dimensions are given
by HEIGHT or WIDTH when non-nil, or by the adjusted image size if either CROP is non-nil or WIN is nil. Image placement is given
by IMAGE-LEFT and IMAGE-TOP when either is non-nil, otherwise is centered in window. IMAGE-LEFT and IMAGE-TOP can either be
specified in pixel coordinates, or by the keywords :LEFT or :RIGHT, or :TOP or :BOTTOM, respectively, thus justifying the image
position with respect to the window as indicated. Place window at LEFT and TOP position in the screen, if included. Window TITLE
defaults to IMAGE, unless a supplied WIN already has a non-nil :TITLE."
  (let* ((path nil)
	 (bitmap-object
	  (cond ((schema-p image) image)
		(t (setq path (if image (pathname image) (file-browser "Find Image File" "/home" '("xbm"))))
    ...
    (when bitmap-object
      (let* ((win (or win (create-instance nil inter:interactor-window (:title (string (or title image-name)))
	     ...
	     (image-width (+ (gv bitmap-object :width) (if HORIZONTAL-SIDE-BORDER (* 2 HORIZONTAL-SIDE-BORDER) 0)))
	     (image-height (+ (gv bitmap-object :height) (if VERTICAL-SIDE-BORDER (* 2 VERTICAL-SIDE-BORDER) 0))))
	...
	(s-value bitmap-object :left (o-formula (round (case (gvl :image-left)
							 (:left 0)
							 (:right (- (gvl :window :width) (gvl :width)))
							 (t (or (gvl :image-left) (- (/ (gvl :window :width) 2) (/ (gvl :width) 2))))))))
	(s-value bitmap-object :top (o-formula (round (case (gvl :image-top)
							(:top 0)
							(:bottom (- (gvl :window :height) (+ (or VERTICAL-SIDE-BORDER 0) (gvl :height))))
							(t (or (gvl :IMAGE-top) (- (/ (gvl :window :height) 2) (/ (gvl :height) 2))))))))
