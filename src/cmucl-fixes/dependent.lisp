...
#+cmu
(ext:file-comment
  "$Header: /home/CVS-cmucl/src/clx/dependent.lisp,v 1.11 1999/03/16 23:37:39 pw Exp $")
...
(declaim (notinline card8->int8 int8->card8
...
(declaim (notinline aref-card8 aset-card8 aref-int8 aset-int8))
...
(defun aset-card16 (v a i)
  (declare (type card16 v)
	   (type buffer-bytes a)
	   (type array-index i))
  (declare (optimize (safety 0) (speed 3) (space 1))) ;   #.(declare-buffun)
...
(defun xlib::radians->int16 (value)
  ...
  (declare (optimize (safety 0) (speed 3) (space 1)))
  (the xlib::int16 (values (round (the xlib::angle value)
			    #.(float (/ pi 180.0s0 64.0s0) 0.0s0)))))
...
(defun xlib::radians->int16 (value)
  ...
  (declare (optimize (safety 0) (speed 3) (space 1)))
  (typecase value
    (single-float (kernel:%unary-round
		   (the single-float
		     (/ (the single-float value)
			#.(float (/ pi 180.0s0 64.0s0) 0.0s0)))))
    (t (kernel:%unary-round
	(the double-float
