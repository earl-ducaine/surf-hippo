...
;;; This description of branches is derived from Hines, "Efficient computation of branched nerve equations "Int. J. Bio-Medical
;;; Computing (15) (1984) p69-76. A branch is composed of connected segments. The ends of the branch are determined by those
;;; segments which (at either end of the segment) connect to more than one segment. The branches of a cell, and in turn the branch
;;; segments, are numbered as follows: Choose any branch of the tree which is connected at one end to the soma. Number the
;;; segments of that branch so that the segment connected to the soma is the last segment. The first segment of this branch
;;; ('trunk') will be called the 'branch node'. Note that this branch node is not the same thing as a true circuit node (as
;;; referenced by Surf-Hippo).

;;; The soma is also considered a 'branch node'.

;;; All branches connecting to a branch node have their segments numbered so that their last segment connects to this node. Their
;;; first segments are also called branch nodes (as long as other segments are connected to them), and this segment-numbering
;;; process continues until all the segments are numbered. The last numbered branches, or 'twigs', all have one end (their first
...
;;; Branches are numbered as follows: Assume that there are N branches. Starting at soma, number all the branches connecting to
;;; the soma starting with N and decrementing. Continue numbering all the branches that are connected to the previous set of
;;; branches, decrementing the branch number.  Continue working out on the tree until all the branches are labeled.

;;; The two circuit nodes (proximal and distal) associated with each segment are ordered according to the branch number and the
;;; number of the attached segment in the proximal direction. Note that since the circuit is a tree, there will only be one such
;;; segment for each node, whereas there can be multiple segments attached to a node in the distal direction. For example, if
;;; branch 33 has 2 segments and branch 34 has 3 segments, then the nodes would be ordered: ...33-1, 33-2, 34-1, 34-2, 34-3... The
;;; soma node has the highest node index.


(defun build-branch-list (seg)
  ;; This is called from LOCATE-CELL-NODES, where cell nodes are located seqentially from the soma outward, and from
  ;; GET-AND-LOCATE-DISTAL-SEGMENTS, which is called recursively from LOCATE-CELL-NODES (for each branch originating at the soma).
  (unless (node-has-ideal-voltage-source (segment-node-2 seg))
    (let ((last-seg (caar *branch-list*)) ; Get the last segment added to the *branch-list*
      ...
      (when last-seg
	;; Count the segments attached to the last segment's proximal node. If this is greater than 2, then this segment was a node of the last branch.
...
;;; **** Solving the System **** Upper triangularization of the matrix is accomplished by repeated applications of the procedure
;;; TRIANG. Define a 'twig' as any branch whose distal end is *not* connected to an un-TRIANGed branch (including no branch at
;;; all). Now upper triangularize the matrix by doing the following until there are only twigs left: Collect the set of all
;;; current twigs, except any twigs which have already been TRIANGed. Apply TRIANG to the remaining set of twigs, in the order
;;; that they appear in the matrix(?). Repeat.
...
(defun TRIANG (branch last-seg)
  ...
  ;; 'branch' is a list of segments in the branch, in order of their node indexes. Note that for the soma branch, the soma will be
  ;; skipped. We want to go from second segment of branch to last segment. If branch has only one segment, then process that one
  ;; at least for its branch-node-index.
  (let* (
...
;; NG's version
(defun TRIANG (branch last-seg)
  ;; NG, j'ai rajoute l'option de compilation-speed a 0, 31/10/97 ca fait gagner 10%.
  (declare (optimize (safety 0) (speed 3) (space 1) (debug 0) (compilation-speed 0))
	   (cons branch) 
	   (type (vector double-float) *diag-double* *lower-diag-double* *upper-diag-double* *rhs-double*))
  ;; 'branch' is a list of segments in the branch, in order of their node indexes. Note that for the soma branch, the soma will be
  ;; skipped. We want to go from second segment of branch to last segment. If branch has only one segment, then process that one
  ;; at least for its branch-node-index.
...
(defun BKSUB (reverse-branch)
  (declare (optimize (safety 0) (speed 3) (space 1) (debug 0) (compilation-speed 0))
	   (cons reverse-branch)
	   (type (vector double-float) *diag-double* *upper-diag-double* *rhs-double* *v-at-half-step-double*))
  ;; 'branch' is a list of segments in the branch, in order.
  (do* ((first-time t nil)
	(rev-branch reverse-branch (cdr rev-branch))
	(segment (car rev-branch) (car rev-branch)))
...
(defun HINES-SOLVE ()
  (declare (optimize (safety 0) (speed 3) (debug 0) (space 0) (compilation-speed 0))
	   ...
	   (type (vector cons) *branch-array* *reverse-branch-array*)
	   (type (vector segment) *last-seg-branch-array*))
  (do ((i 0 (1+ (the fn i))))
  ...
  (do ((i 0 (1+ (the fn i))))
    ...
    (let ((soma (aref (the (vector soma) *soma-array*) i)))
...
(defun HINES-SOLVE ()
  (declare (optimize (safety 0) (speed 3) (debug 0) (space 1) (compilation-speed 0))
  	   (type (vector double-float) *diag-double* *rhs-double* *v-at-half-step-double*)
	   (type (vector cons) *branch-array* *reverse-branch-array*)
	   (type (vector segment) *last-seg-branch-array*))
  ...
  (do ((i 0 (ext:truly-the fn (1+ (ext:truly-the fn i)))))
    ...
    (declare (fixnum i))
    (let ((soma-index (node-index (soma-node (aref (the (vector soma) *soma-array*) i)))))
	...
	do (loop for segment in branch do
		 (let ((nd (segment-node-2 segment)))
		   (unless (node-has-ideal-voltage-source nd)
		     ...
		     (setf (aref (the (vector t) *core-node-array*) *num-unknowns*) nd)
