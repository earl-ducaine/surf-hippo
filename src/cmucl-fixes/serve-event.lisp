...
;;;
(ext:file-comment
  "$Header: serve-event.lisp,v 1.21 93/10/20 13:54:32 ram Exp $")
;;;
;;; **********************************************************************
;;;
...
;;  (format t "           Entering add-fd-handler ...")
  ...
  (let ((handler (make-handler direction fd function)))
    ...
    ;; LBG
;;    (format t "  Exiting add-fd-handler with ~A~%" handler)
    handler))
...
;;; the meantime.
;;;
;; (plot-scatter (loop for x from 1 to 10000 collect (list (random 1.0) (random 1.0))))
;; (plot-timed-data (loop for x from 1 to 10000 collect  (random 1.0)))
; (defvar *wait-until-fd-usable-level* 0)
...
;  (format t "WAIT-UNTIL-FD-USABLE timeout ~A, level ~A " timeout *wait-until-fd-usable-level*)
;  (incf *wait-until-fd-usable-level*)
  (let (; (timeout (or timeout 10)) 				;; LBG 06.09.00
	usable)
    (multiple-value-bind (to-sec to-usec)
			 (decode-timeout timeout)
      (declare (type (or index null) to-sec to-usec))
;      (format t "   to-sec ~a, to-usec ~a~%" to-sec to-usec)
      (multiple-value-bind
	  (stop-sec stop-usec)
	  (if to-sec
		...
		(declare (ignore okay))
;		(format t "      start-sec ~a, start-usec ~a~%" start-sec start-usec)
...
;;	(format t "  Entering with-fd-handler..~%")
	(with-fd-handler (fd direction #'(lambda (fd)
					   (declare (ignore fd))
					   (setf usable t)))
			 (progn
;			 (format t "     Starting with-fd-handler loop..~%")
	  (loop
	   
;	    (format t "                 sub-serve-event to-sec ~a to-usec ~a, Returns~%      " to-sec to-usec)
;	    (format t "~A~%" (sub-serve-event to-sec to-usec))
	    (sub-serve-event to-sec to-usec)
	    
	    (when usable
	      
;	      (decf *wait-until-fd-usable-level*)
;	      (format t "..........Returning from WAIT-UNTIL-FD-USABLE timeout ~A with T level ~a~%" timeout *wait-until-fd-usable-level*)
...
;		  (decf *wait-until-fd-usable-level*)
;		  (format t "..........Returning from WAIT-UNTIL-FD-USABLE timeout ~A with NIL level ~a~%" timeout *wait-until-fd-usable-level*)
...
;; LG May 20, 2002
(defvar *debug-sub-serve-event* nil)
(export '(*debug-sub-serve-event*))

(defun sub-serve-event (to-sec to-usec)
  (when *debug-sub-serve-event* (format t "WITHIN sub-serve-event...."))
  (when (handle-queued-clx-event)
    (when *debug-sub-serve-event* (format t "  --> handle-queued-clx-event....~%"))
    (return-from sub-serve-event t))
  (when *debug-sub-serve-event* (format t "  --> NOT handle-queued-clx-event....~%"))
  ...
  (alien:with-alien ((read-fds (alien:struct unix:fd-set))
		    ...
		    (let ((count (calc-masks)))
		      (when *debug-sub-serve-event* (format t "    -->  with-alien with count ~a, Calling unix-fast-select ....~%" count))
		      (multiple-value-bind
			  (value err)
			  (unix:unix-fast-select
			   ...
			   nil to-sec to-usec)
			(when *debug-sub-serve-event* (format t "    -->  unix:unix-fast-select gave value ~a err ~a ....~%" value err))
			...
			(cond (value
			      ...
			      (t
			       ...
			       nil)))))
  (when *debug-sub-serve-event* (format t " ***** Leaving sub-serve-event....~%")))
