...
(defun get-uniform-xy-variants (&optional (number-of-samples 1))
  ;; Return a list with NUMBER-OF-SAMPLES [default 1] lists of single
  ;; float pairs, each of which is a uniform variant over 0.0 and 1.0.
...
(defun poisson-events (lambda start stop &optional (min-interval-value 0.0))
  "Returns a list of times generated by a Poisson process with rate
   constant LAMBDA [1/ms], starting at START and ending with STOP
   [ms]. Poisson intervals are taken at a minimum value of
...
(defun modulated-poisson-events (LAMBDA-SPEC start stop &key (step 1.0) (time-offset 0.0) (min-interval-value 0.0) (lambda-coefficient 1.0))
  "Returns a list of event times generated by a Poisson process with a
   modulated rate constant lambda over a total interval given by START
   and STOP [ms]. Calculation of lambda at any given time depends on
   LAMBDA-SPEC. When LAMBDA-SPEC is a function, lambda is given by a
   funcall of LAMBDA-SPEC with the time as the argument \(this
   function should take a single numeric argument and return a
   single-float\). When LAMBDA-SPEC is a sequence lambda is the
   interpolated value of LAMBDA-SPEC appropriate for the current
   time. Finally, if LAMBDA-SPEC is a number this gives directly the
   constant value of lambda. In all cases, the evaluation of
   LAMBDA-SPEC is taken to be in 1/ms.  When LAMBDA-SPEC is a
   function, STEP [ms] should be a value for which the function is
   relatively constant. When LAMBDA-SPEC is a sequence, then STEP is
   the time base. If LAMBDA-SPEC is a number, then the list of events
   is generated by the function POISSON-EVENTS, with the START and
   STOP arguments adjusted by the value of TIME-OFFSET [ms, default
   0.0]. Otherwise, the events are generated by repeated calls to
   POISSON-INTERVAL, with the initial time given by START, and until
   time reaches STOP. At every time increment if POISSON-INTERVAL
   gives a value greater than STEP, then the time is incremented by
   STEP and the process repeated. If the returned poisson interval is
   less than STEP, then the time is incremented by the interval, and
   this value is pushed onto the result list. All times in the
   returned list are adjusted by the addition of TIME-OFFSET [ms].
   Poisson intervals are taken at a minimum value of
   MIN-INTERVAL-VALUE [ms]."
...
(defun get-random-phase-sequence-phase (freq low-cutoff high-cutoff)
  (declare (optimize (safety 0) (speed 3) (space 0))
	   (single-float freq low-cutoff high-cutoff))
  (if (not (>= high-cutoff freq low-cutoff)) 0.0 (- (random (* 2 pi-single)) pi-single)))

(proclaim '(inline get-random-phase-sequence-mag))
(defun get-random-phase-sequence-mag (mag-function freq low-cutoff high-cutoff)
  (declare (optimize (safety 0) (speed 3) (space 0))
	   (single-float freq low-cutoff high-cutoff))
...
(defun random-phase-sequence-half-real-and-imag-no-dc-to-arrays
  (real-array imag-array delta-hz inner-duration low-cutoff high-cutoff mag-function max-freq ac-coupled plot-mag-phase)
  (declare (optimize (safety 0) (speed 3) (space 0))
	   (single-float max-freq delta-hz low-cutoff high-cutoff)
	   (fixnum inner-duration)
	   (type (vector single-float) real-array imag-array))
  (flet ((get-random-phase (freq low-cutoff high-cutoff)
	 ...
	 (get-mag (mag-function freq low-cutoff high-cutoff)
		  (if (not (>= high-cutoff freq low-cutoff)) 0.0 (case mag-function
								   ...
								   (:1/f (/ 1 freq))
								   (:1/F-SQUARED (/ 1 (* freq freq)))))))
    (let* ((dc-mag (if (or ac-coupled (> low-cutoff 0)) 0.0 1.0))
	   (max-freq-mag (get-mag mag-function max-freq low-cutoff high-cutoff))
	   (max-freq-phase (get-random-phase max-freq low-cutoff high-cutoff))
	   (ss-freq-mag (+ (square dc-mag) (square max-freq-mag))) ; Start sum of freq_mag^2
	   reverse-half-of-mag-no-dc reverse-half-of-phase-no-dc) ; only used if plotting mag phase.
      (declare (single-float dc-mag max-freq-mag max-freq-phase ss-freq-mag))
      (setf (aref real-array 0) dc-mag)
      (setf (aref imag-array 0) 0.0)	; dc-phase implicitely is 0.0
      (loop for freq single-float from delta-hz by delta-hz
	    for count fixnum from 1 to (1- (the fn (/ (the fn inner-duration) 2)))
	    do (let* ((mag (get-random-phase-sequence-mag mag-function freq low-cutoff high-cutoff))
		      (phase (get-random-phase-sequence-phase freq low-cutoff high-cutoff))
		      (real (real-from-mag-phase mag phase))
		      (imag (imag-from-mag-phase mag phase)))
		 (declare (single-float mag phase real imag))
		 (when plot-mag-phase (push mag reverse-half-of-mag-no-dc) (push phase reverse-half-of-phase-no-dc))
		 (incf ss-freq-mag (* 2 (square mag)))
		 (setf (aref real-array count) real)  (setf (aref real-array (- inner-duration count)) real)
		 (setf (aref imag-array count) imag)  (setf (aref imag-array (- inner-duration count)) (- imag)))
	    finally (let* ((real-at-f_max (real-from-mag-phase max-freq-mag max-freq-phase))
			   (imag-at-f_max (imag-from-mag-phase max-freq-mag max-freq-phase)))
		      (setf (aref real-array (1+ count)) real-at-f_max)
		      (setf (aref imag-array (1+ count)) imag-at-f_max)))
      (values ss-freq-mag max-freq-mag max-freq-phase reverse-half-of-mag-no-dc reverse-half-of-phase-no-dc))))
...
(defun foo ()
  (let ((*preserve-plot-layout* t))
    (dotimes (i 30 )
      (RANDOM-PHASE-SEQUENCE 1000 :plot-result t :mag-function :1/f :low-cutoff 10 :high-cutoff 200 :plot-mag-phase nil :delta-t 0.5 :VERBOSE-PLOT-TITLES nil))))
|#

(defun random-phase-sequence (duration	; ms
			      &key (low-cutoff 0) high-cutoff ; hz
			      (delta-t 1.0) ; ms
			      (mag-function :FLAT) ; Also :1/F, :1/F-SQUARED
			      (verbose-plot-titles t) (rms 1) (rms-on-infinite-sequence t) (ac-coupled t) plot-mag-phase plot-result plot-dft-sequences return-list)
  "Generate a single float list of length [maximum (expt 2 16)] given by the ratio of DURATION and DELTA-T, the latter in units of ms [default 1], whose
frequency spectrum magnitude is given by MAG-FUNCTION - :FLAT [default], :1/F or :1/F-SQUARED - between LOW-CUTOFF [default 0] and HIGH-CUTOFF [default
NIL, both in hz when numeric], otherwise 0, and whose phase is given by a random deviate with flat distribution between -Pi and Pi. When HIGH-CUTOFF is NIL
then there is no low pass characteristic. The DC component is included only if AC-COUPLED is NIL [default T]. If RETURN-LIST is T [default NIL] the list is
returned, and if PLOT-RESULT is T [default NIL] the output plotted. Intermediate results are plotted if PLOT-DFT-SEQUENCES is T [default NIL]. The average
power in the output is given by RMS, which corresponds to the standard deviation of the distribution. This constraint is applied on the corresponding
infinite output sequence if RMS-ON-INFINITE-SEQUENCE is T [default], otherwise on the actual finite length output sequence. For reasonable reproduction of
the highest frequency components in the output list, the product of DELTA-T and the maximum frequency with significant power in the passband should be at
most 100. VERBOSE-PLOT-TITLES [default T] enables spectrum information added to non-DFT plot window titles."
  ...
  (let* ((delta-t (s-flt delta-t))
	 ...
	 (real-input (make-array (list inner-duration) :element-type 'single-float))
	 (imag-input (make-array (list inner-duration) :element-type 'single-float)))
    (declare (fixnum duration output-time-steps))
    (when (> output-time-steps (expt 2 16))
      (sim-error (format nil "DFT processing can only handle total sequence length <= 2^16 (now duration/time-step is ~A)" output-time-steps)))
    (multiple-value-bind (ss-freq-mag max-freq-mag max-freq-phase reverse-half-of-mag-no-dc reverse-half-of-phase-no-dc)
	(random-phase-sequence-half-real-and-imag-no-dc-to-arrays
	 real-input imag-input delta-hz inner-duration low-cutoff high-cutoff mag-function max-freq ac-coupled plot-mag-phase)
      (declare (single-float delta-hz delta-t max-freq high-cutoff low-cutoff)
	       (type (vector single-float) real-input imag-input)
	       (fixnum output-time-steps inner-duration))
      (multiple-value-bind (real-output) ; ignoring IMAG-OUTPUT
	  (dft-reverse-complete-basic real-input imag-input)
	(declare (type (vector single-float) real-output))
	(let* ((rms-infinite-output (/ (the sf (sqrt ss-freq-mag)) inner-duration)) ; This derives from Parseval's theorem.
	       (rms-actual-output (rms-sf real-output))
	       (gain (if (zerop rms-actual-output) 1.0 (/ rms (if RMS-ON-INFINITE-SEQUENCE rms-infinite-output rms-actual-output))))
	       (actual-output (loop for val single-float across real-output
				    for index fixnum from 0 to (1- output-time-steps)
				    collect (if (= gain 1) val (* gain val)))))	; Adjust output amplitude if necessary.
	  (declare (single-float rms-actual-output rms-infinite-output ss-freq-mag gain))
	  (random-phase-sequence-plot VERBOSE-PLOT-TITLES plot-mag-phase plot-result ACTUAL-OUTPUT DELTA-T DURATION
				      mag-function gain ac-coupled low-cutoff high-cutoff
				      delta-hz max-freq reverse-half-of-mag-no-dc max-freq-mag reverse-half-of-phase-no-dc max-freq-phase)
	  (when return-list actual-output))))))

(defun random-phase-plot-title (VERBOSE-PLOT-TITLES mag-function ac-coupled low-cutoff high-cutoff max-freq &optional extra)
...
#|
(defun random-phase-sequence-plot (VERBOSE-PLOT-TITLES plot-mag-phase plot-result ACTUAL-OUTPUT DELTA-T DURATION
						       mag-function gain ac-coupled low-cutoff high-cutoff
						       delta-hz max-freq reverse-half-of-mag-no-dc max-freq-mag reverse-half-of-phase-no-dc
						       max-freq-phase)
  (when plot-result
    (plot-timed-data actual-output nil delta-t :x-are-fns t :x-label "ms" :x-max duration :x-label-h-position :right
		     :title (random-phase-plot-title VERBOSE-PLOT-TITLES mag-function ac-coupled low-cutoff high-cutoff max-freq)))
  (when plot-mag-phase
    (let ((mag-input (scale-float-list
		      gain
		      (concatenate 'list `(,(if (or ac-coupled (> low-cutoff 0)) 0.0 1.0)) ; dc mag
				   (reverse reverse-half-of-mag-no-dc) (list max-freq-mag) reverse-half-of-mag-no-dc)))
	  (phase-input (concatenate 'list '(0.0) (reverse reverse-half-of-phase-no-dc)
				    (list max-freq-phase) (negate-single-float reverse-half-of-phase-no-dc))))
      (plot-timed-data (fold-wave mag-input) nil delta-hz :delta-t-start (round (- max-freq))
		       ...
		       :y-label "Mag" :x-label "Hz" :x-label-h-position :right :x-are-fns t
		       :title (random-phase-plot-title VERBOSE-PLOT-TITLES mag-function ac-coupled low-cutoff high-cutoff max-freq "Mag Input"))
      (plot-timed-data (fold-wave phase-input) nil delta-hz :delta-t-start (round (- max-freq))
		       ...
		       :default-number-of-x-incs 10 :y-label "Phase" :x-label "Hz" :x-label-h-position :right :x-are-fns t
		       :title (random-phase-plot-title VERBOSE-PLOT-TITLES mag-function ac-coupled low-cutoff high-cutoff max-freq "Phase Input")))))
...
(let ((step 0.0010d0)
  ...
  (loop for x double-float from (- max) to max by step
	collect (* 0.5 (+ 1 (rc:erf x))) into erf
...
(defun normal-random-number ()
  ...
  (declare (optimize (safety 0) (speed 3) (space 1)))
  (the df (aref (the (vector DOUBLE-FLOAT) *erf-array-argument*) (array-search-double (the df (random 1.0d0)) *erf-array*))))

;; Modified from Royland code - make sure not taking log 0.
(defun NORMAL-RANDOM-NUMBER (&optional (mean 0.0d0) (variance 0.5d0))
  "Return a double-float random number taken from a normal distribution with MEAN [default 0.0d0] and VARIANCE
 [default 0.5d0]. MEAN and VARIANCE must be double-floats"
  (declare (optimize (safety 0) (speed 3) (space 1))
	   (double-float mean variance))
  (let* ((normal
	  (do ((u 0.0)
	       (v 0.0))
	      ((progn
		 (setq u (user::uniform-random-number) ; U is bounded (0 1)
		       v (* 2.0 (sqrt 2.0) (exp -0.5) ; V is bounded (-MAX MAX)
			    (- (user::uniform-random-number) 0.5)))
		 (and (not (zerop u))
		      (<= (* v v) (* -4.0 u u (the sf (log u)))))) ; < should be <=
	       (/ v u))
	    (declare (float u v))))
...
(defun test-normal-random-number (mean variance)
  (let ((mean (d-flt mean))
    ...
    (plot-histogram (loop for count from 1 to 100000 collect (normal-random-number mean variance))
		    :x-axis-tick-mark-skip 11
		    :x-axis-tick-skip 11
		    :x-are-fns nil
...
#|
(plot-histogram (loop for x from 1 to 10000 collect (aref *erf-array-argument* (array-search (random 1.0d0) *erf-array*)))
		:bin-width 0.025 :bins 240 :x-min -3 :x-max 3 :x-axis-tick-skip 9)
(plot-histogram (loop for x from 1 to 500 collect (aref *erf-array-argument* (simple-array-search (random 1.0d0) *erf-array*)))
		     :bin-width 0.025 :bins 240 :x-min -3 :x-max 3 :x-axis-tick-skip 9)
(plot-timed-data (loop for x from 0 to 1.0 by 0.001 collect (aref *erf-array-argument* (array-search x *erf-array*))))
(plot-timed-data (loop for x from 0 to 1.0 by 0.001 collect (aref *erf-array* (array-search x *erf-array*))))
...
(defun RANDOM-NTH (list &optional (total 1))
  ...
  (declare (optimize (safety 1) (speed 3) (space 0))
	   (type (or NULL cons) list))
  (when list
    (let ((total (round total)))
      (if (> total (length list))
	(sim-error (format nil "RANDOM-NTH: TOTAL of ~A is more than the list length ~A!" total (length list)))
	(loop until (= (length out) total)
	      with temp
	      do (setq temp (nth (random (the (INTEGER 1 524287) (length list))) list))
	      unless (member temp out)
	      collect temp into out
	      finally (return (atomize-list out)))))))

(defun RANDOM-NTH-fraction (list &optional (fraction 1.0))
