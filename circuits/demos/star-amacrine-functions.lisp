...
retinal directional selectivity described in 

  Borg-Graham, L. and Grzywacz, N. M., A Model of the direction selectivity circuit in retina:
  Transformations by neurons singly and in concert. In Single Neuron Computation, edited by T.
  McKenna, J. Davis, and S. F. Zornetzer, pp347-376. Academic Press, 1992.

|#

;; The Na and K channels do *not* describe actual retinal channels, but rather act to generate caricature spiking somatic properties.
(channel-type-def
 '(dr-RET-hh-ext
   (gbar-density . 360.0)
   (e-rev . -77.0)
...
(defun star-amacrine-demo ()
  ...
  (run)
  (just-draw :scale 1.5 :draw-light-stimulus t)
  (set-color-map :map 'gray-rgb-map)
  (replay-colorized-simulation :time-step 0.2 :repetitions 4 :include-colorizing-scale t))
  

;; Now run (GOFERIT), or for a more complete display, run the function (STAR-AMACRINE-DEMO) defined above.




;; Plot the integral of the voltage response in the distal nodes with this function, according to their angle.
