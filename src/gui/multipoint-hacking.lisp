...
(defun new-add-polyline-to-plot (plot-agg points line-style connect-ends)
  (loop for points in (massage-points points) do
	(opal:add-component           
	 plot-agg (create-instance nil (if (g-value plot-agg :window :plot-POLYLINE-as-multipoint) opal:multipoint opal:polyline)
				   ;; (:constant t)
...
(defun add-polyline-to-plot (plot-agg plot-win x-list y-list line-style &key connect-ends)
  ...
  (opal:add-component			;Data Points
   plot-agg		 
   (create-instance nil (if (g-value plot-agg :window :plot-POLYLINE-as-multipoint) opal:multipoint opal:polyline)
		    ;; (:constant t)
...
(defun foo ()
  (let* ((count 10000)
    ...
    (plot-xy-data random-dots-1 nil  :width 500 :height 500
		  ; :connect-data-points nil :scatter t :x-symbol-width 2
